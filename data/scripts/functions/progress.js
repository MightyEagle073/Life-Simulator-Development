...
function progress() {
    if (breakfn == 0) {
        ...
        if (lifeInfo.dsb == 0) {
            diary_str1 = convert_dict_date(lifeInfo.date)
            diary_str2 = " - I have been brought into this world. <br>"
            lifeInfo.diary = diary_str1 + diary_str2;
        }
        ...
        if (Math.pow(10, lifeInfo.dsb * 0.0001) >= 10000000 * death_x) {
            ...
            lifeInfo.status = 2;
            console.log(
                `Dead at ${lifeInfo.age.years} years ${lifeInfo.age.days} days ` +
                `due to a death_x of ${death_x.toString()}`
            );
            diaryAdd(
                `I died due to natural causes. I was aged ${lifeInfo.age.years} years and ` +
                `${lifeInfo.age.days} days when I died.`
            );
            playAudio("death");
            displayType("death_overlay", "block");
            $("#death_died").html(
                `${lifeInfo.name.first} ${lifeInfo.name.last} has died on` +
                `${convert_dict_date(lifeInfo.date)} due to natural causes.`
            );
            switch (lifeInfo.gender) {
                case "m":
                    $("#death_age").html("He was at an age of " + lifeInfo.age.years + 
                    " years " + lifeInfo.age.days + " days.");
                    break;
                case "f":
                    $("#death_age").html("She was at an age of " + lifeInfo.age.years + 
                    " years " + lifeInfo.age.days + " days.");
                    break;
            }
        }
        ...
        $("#main_info_age").html(
            "Age: " + lifeInfo.age.years + " years " + lifeInfo.age.days + " days"
        );
        $("#main_info_balance").html(
            "Balance: " + 
            lifeInfo.balance.toLocaleString("en-AU", {style: "currency", currency: "AUD"})
            );
        $("#main_info_netWorth").html(
            "Net Worth: " + 
            lifeInfo.netWorth.toLocaleString("en-AU", {style: "currency", currency: "AUD"})
            );
